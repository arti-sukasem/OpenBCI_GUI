...
class W_Gastric extends Widget {

    //to see all core variables/methods of the Widget class, refer to Widget.pde
    //put your custom variables here...
    Button widgetTemplateButton;

    W_Gastric(PApplet _parent){
        super(_parent); //calls the parent CONSTRUCTOR method of Widget (DON'T REMOVE)

        //This is the protocol for setting up dropdowns.
        //Note that these 3 dropdowns correspond to the 3 global functions below
        //You just need to make sure the "id" (the 1st String) has the same name as the corresponding function
        addDropdown("Dropdown12", "Drop 1", Arrays.asList("A", "B"), 0);
        addDropdown("Dropdown22", "Drop 2", Arrays.asList("C", "D", "E"), 1);
        addDropdown("Dropdown23", "Drop 3", Arrays.asList("F", "G", "H", "I"), 3);
...
void Dropdown22(int n){
    println("Item " + (n+1) + " selected from Dropdown 2");
    closeAllDropdowns();
}

void Dropdown23(int n){
